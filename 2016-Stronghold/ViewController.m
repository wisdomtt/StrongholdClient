...
- (void)viewDidLoad {
    ...
    // Do any additional setup after loading the view, typically from a nib.
        ...
        NSString *dataPath = [documentsDirectory stringByAppendingPathComponent:@"/FRCScouting"];
...
//[object SEND:@"HAI"];
    ...
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0]; // Get documents folder
    documentsDirectory = [NSString stringWithFormat:@"%@/FRCScouting", documentsDirectory];
...
- (IBAction)Save:(id)sender {
    NSLog(@"%@", object.session.connectedPeers);
    //Save Stuff
    [self Calculate];
...
//            }
       ...
       // NSLog(@"I again: %d", i);
       // NSLog(@"%@", fileContents[d]);
        NSLog(@"[D] %d", d);
        NSLog(@"[PATH] %@", sPath);
        
        //NSLog(@"%@", [fileContents count]));
        if(d<contentcount)
